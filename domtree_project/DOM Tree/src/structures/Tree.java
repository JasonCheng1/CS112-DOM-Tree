...
public class Tree {
	...
	private static void recAddTag(String word, String tag, TagNode root, TagNode realRoot){
			if(root == null) {
				...
				TagNode prevPrev = findPrev(prev, realRoot);
				TagNode ptr = root;
				if(/*ptr != null && prev != null && prevPrev != null && */ prev.firstChild == ptr && prevPrev.firstChild == prev && (prevPrev.tag.equals("em") || prevPrev.tag.equals("b")) && (prev.tag.equals("em") || prev.tag.equals("b"))) {
					return;
				}
				if(prev.tag.equals(tag) && (prev.firstChild == root)) {//doesn't work for successive cases
					return;
				}
